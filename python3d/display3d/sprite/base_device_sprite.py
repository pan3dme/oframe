...
class BaseDeviceSprite3D(BaseScaleBallSprite3D):

    def __init__(self, scene, text, color: tuple = (1.0, 1.0, 1.0)):
        super().__init__(scene, color)

        self.text_sprite = BaseTextSprite3D(self.scene3D, text=text, font_size=64, color=(1.0, 1.0, 1.0))
        self.text_sprite.scaleX = 2
        self.text_sprite.scaleY = 2
        self.text_sprite.scaleZ = 2
