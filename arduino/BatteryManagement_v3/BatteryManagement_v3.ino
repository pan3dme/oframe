...
#define RF_FREQUENCY 928000000  // 频率：433MHz（国内通用）

#define TX_OUTPUT_POWER 10  // 发射功率：10dBm（近距离不用大）

#define LORA_BANDWIDTH 0          // 带宽：0=125kHz（最稳定）
#define LORA_SPREADING_FACTOR 11  // SF7：近距离最快最稳（关键！）
...
#define VBAT_CTRL_PIN 37  // ADC_Ctrl 引脚（控制检测电路开关）
#define VBAT_READ_PIN 1   // VBAT_Read 引脚（ADC1_CH0）
...
void setup() {
  ...
  analogReadResolution(12);  // ESP32-S3 ADC 12位分辨率
...
float readBatteryVoltage() {
  ...
  // V3 内部分压 1/2 → 真实电压 = ADC电压 ×2
  float batteryVoltage = mvAvg * 4.0 / 1000.0;
...
String getBatterInfo() {
  ...
  int soc = map(batVoltage * 1000, 3000, 4200, 0, 100);
  soc = constrain(soc, 0, 100);
  return "v:" + String(batVoltage, 2) + " " + String(soc) + "%";
}
...
void loop() {
  ...
  Serial.println(getBatterInfo());
  // ============== 每10秒执行一次：打印系统状态 ==============
  if (millis() - lastPrintTime >= printInterval) {
    batteryStr=getBatterInfo();
  ...
  switch (state) {
    ...
    case STATE_TX:
      delay(1000);  // 延时1秒再发送
      sprintf(txpacket, "%s|%s", deviceName, batteryStr);
